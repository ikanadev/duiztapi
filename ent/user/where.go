...
// HasQuizes applies the HasEdge predicate on the "quizes" edge.
func HasQuizes() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(QuizesTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, QuizesTable, QuizesPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasQuizesWith applies the HasEdge predicate on the "quizes" edge with a given conditions (other predicates).
func HasQuizesWith(preds ...predicate.Quiz) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(QuizesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, QuizesTable, QuizesPrimaryKey...),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
