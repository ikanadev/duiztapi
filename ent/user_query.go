...
func (uq *UserQuery) QueryResponses() *ResponseQuery {
	query := &ResponseQuery{config: uq.config}
...
// QueryQuizes chains the current query on the "quizes" edge.
func (uq *UserQuery) QueryQuizes() *QuizQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withQuizes:    uq.withQuizes.Clone(),
...
// WithQuizes tells the query-builder to eager-load the nodes that are connected to
// the "quizes" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithQuizes(opts ...func(*QuizQuery)) *UserQuery {
	query := &QuizQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withResponses; query != nil {
		...
		query.Where(predicate.Response(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ResponsesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_responses
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_responses" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_responses" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Responses = append(node.Edges.Responses, n)
		}
	}

	if query := uq.withQuizes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Quizes = []*Quiz{}
		}
		var (
			edgeids []int
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.QuizesPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "quizes": %v`, err)
		}
		query.Where(quiz.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "quizes" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Quizes = append(nodes[i].Edges.Quizes, n)
			}
		}
	}
